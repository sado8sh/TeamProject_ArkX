...
function applyFilters() {
  ...
  const likedFilter = document.getElementById("likedFilter");
  const container = document.getElementById("car-section");
  ...
  const likedOnly = likedFilter.checked;

  cards.forEach(card => {
    // Find owner text in card
    const ownerText = card.querySelector(".card-content p strong").parentElement.textContent || "";
    const owner = ownerText.replace("Chosen by:", "").trim();

    // Find if liked button is active
    const isLiked = card.querySelector("button").classList.contains("liked");

    // Decide if card should be visible
    const showByOwner = selectedOwner === "all" || owner === selectedOwner;
    const showByLiked = !likedOnly || isLiked;

    if (showByOwner && showByLiked) {
...
// Add event listeners to filter inputs (run this once DOM is ready)
